...
export class OfflineService {
	...
	async processQueue(): Promise<{
		...
		try {
			...
			console.log(`[OfflineService] Processing ${pendingItems.length} pending items`);
			...
			for (const item of pendingItems) {
				try {
					...
					console.error(`[OfflineService] Error processing item ${item.id}:`, err);
					failed++;
				}
			}
		} finally {
	...
	async resolveConflict(
		...
		if (!item) {
			console.error('[OfflineService] Queue item not found for conflict resolution');
			return;
		}
	...
	async clearQueue(): Promise<void> {
		await this.db.syncQueue.where('status').anyOf(['completed', 'failed']).delete();
	}
}
