...
vi.mock('$lib/stores/teams', () => ({
  currentTeam: createStoreMock(mockCurrentTeam),
}));
...
describe('taskService', () => {
  ...
  describe('listAll', () => {
    it('should return all tasks for current team', async () => {
      const { taskService } = await import('$lib/api/services/taskService');
      const mockTasks = [
    ...
    it('should filter by completed status', async () => {
      ...
      const mockTasks = [
        createMockTask({
          id: 'task1',
          task_stages: { ...createMockTask().task_stages, is_completion_stage: true },
        }),
      ];

      // The query starts with .from().select().is().order()
      // Then it's reassigned: query = query.eq(...)
      // So we need order() to return a chainable object, not a promise
      const baseQuery = createQueryBuilder({
        data: mockTasks,
        error: null,
      });
      
      // Override order to return a chainable object (not a promise)
      // This allows query = query.eq(...) to work
      baseQuery.order = vi.fn(() => baseQuery);

      mockSupabase.from = vi.fn(() => baseQuery);

      const tasks = await taskService.listAll({ completed: true });

      expect(tasks).toHaveLength(1);
      expect(baseQuery.eq).toHaveBeenCalledWith('task_stages.is_completion_stage', true);
    });
    ...
    it('should handle errors', async () => {
      ...
      await expect(taskService.listAll()).rejects.toEqual({ message: 'Database error' });
    });
  });
  ...
  describe('create', () => {
    it('should create a new task with required fields', async () => {
      const { taskService } = await import('$lib/api/services/taskService');
      const { taskStageService } = await import('$lib/api/services/taskStageService');
      ...
      vi.mocked(taskStageService.list).mockResolvedValue(mockStages as any);
    ...
    it('should throw error if no team is selected', async () => {
      const { taskService } = await import('$lib/api/services/taskService');
      const { currentTeam } = await import('$lib/stores/teams');
      ...
      await expect(taskService.create(taskData)).rejects.toThrow('No active team selected');
      
      // Restore mock team
      currentTeam.set(mockCurrentTeam);
    });
