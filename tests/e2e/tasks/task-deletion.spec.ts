...
test.describe('Task Deletion with Dependencies', () => {
	...
	test('should show confirmation modal when deleting task without dependencies', async ({ page }) => {
	...
	test('should show dependency count in confirmation modal', async ({ page }) => {
	...
	test('should update dependency count after removing subtask', async ({ page }) => {
		...
		const subtask1 = await taskPage.addSubtask(parentTask.id, { title: 'Subtask 1' });
		await taskPage.addSubtask(parentTask.id, { title: 'Subtask 2' });
	...
	test('should preserve deleted_by user ID', async ({ page }) => {
		...
		const response = await page.request.get(`/api/tasks/${task.id}?include_deleted=true`);
...
test.describe('Task Deletion Permissions', () => {
	test('should only allow task creator or admin to delete', async ({ page }) => {
