...
- Q: How should container node details be displayed when users drill into containers to prevent context loss? → A: Inspector panel (desktop: right sidebar, mobile: bottom drawer) + compact context bar at top showing parent container metadata
- Q: Should NodeCard and ReferenceCard remain separate or unify into one component? → A: Unified component with expand/collapse functionality and user preferences for default density (compact/expanded)
- Q: How should context about parent containers be shown when inside a container's moodboard? → A: Compact context bar at the top showing container name, type, and key metadata (character name, series, variant) with breadcrumb navigation
- Q: Should References Tab and Moodboard be unified or kept separate? → A: Replace References Tab with Moodboard Tab using the same unified component everywhere, with smart responsive defaults (canvas for desktop, gallery for mobile)
- Q: What is the intended purpose and flexibility of container nodes? → A: Flexible "sacks/stacks" for any spatial organization need - characters, props, equipment lists, outfit options, event planning. Not limited to characters only
...
### 1.6. Container Nodes & Detail Display 🎯 HIGH PRIORITY

**Concept**: Container nodes are flexible "sacks/stacks" for spatial organization with proper context preservation

**Container Node Purpose**:
- **NOT just for characters** - flexible organizational tool for any spatial grouping:
  - Character options (Link Default, Link BotW, Beedle archived project)
  - Equipment lists (Photographer's PAX gear: "2 lenses" vs "1 lens + lighting setup")
  - Outfit planning (Multiple cons: "PAX East Day 1", "PAX East Day 2", "Anime Boston")
  - Prop collections (Related props for a scene or character)
  - Material groups (Fabrics for specific costume piece)
  
**Container Types**:
- `character` - Character variations and options
- `prop` - Prop collections and variations
- `group` - Generic grouping for any purpose (equipment, outfits, materials, etc.)

**Container Metadata**:
- Containers have details: name, description, custom fields
- Examples:
  - Character: `character_name`, `series_name`, `variant`, `estimated_budget`, `actual_budget`
  - Prop: `prop_name`, `material`, `difficulty`
  - Group: `group_name`, `purpose`, flexible custom fields

**Context Loss Problem & Solution**:
- **Problem**: When user creates container node with details at parent level, then drills into container's moodboard, those details disappear
- **OLD Approach (awkward)**: `container_details` child node - feels wrong because it's metadata ABOUT the space, not content IN the space
- **NEW Approach (approved)**:
  1. **Inspector Panel**: Figma/Notion-style sidebar for editing container metadata
     - **Desktop**: Right sidebar (300-400px) that slides in when container selected
     - **Mobile**: Bottom drawer that slides up (60-80% of screen height)
     - Shows all container fields in editable form
     - Save button or auto-save on blur
     - Can be collapsed/expanded by user
  2. **Context Bar**: Compact header showing parent container context when inside container
     - Always visible at top when inside a container's moodboard
     - Shows: Container icon, name, key metadata (character name, series, variant)
     - Breadcrumb navigation: "All > [Container Name] > Current View"
     - Click breadcrumb to navigate back to parent
     - Compact (40-50px height) to preserve canvas space

**Inspector Panel Features**:
- **Always accessible**: Select any node → Inspector shows its properties
- **Context-aware**: Different fields based on node type
  - Container: Character name, series, variant, budget, custom fields
  - Image: Tags, notes, source URL
  - Social media: Platform, author, tags, notes
  - Budget item: Cost, quantity, supplier, priority
- **Editable**: Inline editing of all fields
- **Attachments**: Link related nodes (e.g., budget item → supplier contact)
- **History**: Show edit history (future enhancement)

**Context Bar Features**:
- **Breadcrumb navigation**: "All > Character: Link (BotW)" - click to go back
- **Key metadata preview**: Shows 2-3 most important fields inline
- **Quick actions**: Edit (opens inspector), Add content, Switch view mode
- **Collapse option**: User can minimize to icon-only mode for max canvas space
- **Mobile-friendly**: Touch-friendly tap targets, swipe gestures

**User Workflow Example**:
1. User on main moodboard, sees container card "Character: Link (BotW)"
2. Container card shows preview: character name, series, estimated budget
3. User clicks container → Drills into container's moodboard
4. **Context bar** appears at top: "All > Character: Link (BotW) | Series: Zelda | Budget: $250"
5. User selects container background → **Inspector panel** slides in from right
6. Inspector shows all fields: character_name, series_name, variant, estimated_budget, custom_fields
7. User edits fields inline, saves
8. User clicks breadcrumb "All" to return to parent moodboard
9. Container card now shows updated metadata

**Unified Card Design**:
- **Single component** used everywhere: ReferenceCard, NodeCard → UnifiedCard
- **Adaptive density**: User preference for default view (compact vs expanded)
  - **Compact**: Small thumbnail, title, 1-2 metadata fields, tags
  - **Expanded**: Large preview, full metadata, notes section, embedded content
- **Expand/collapse toggle**: Icon button on card to switch density on-demand
- **User preferences**: Save default density per view mode (canvas, gallery, list, table)
  - Example: User prefers compact on canvas, expanded in gallery
- **Consistent design**: Same visual language across all contexts
  - Consistent shadows, borders, hover states
  - Platform badges (Instagram red, TikTok black, YouTube red)
  - Tag pills with consistent styling
  - Action buttons (edit, delete, expand, share) in same positions

**References Tab → Moodboard Tab Unification**:
- **Replace** "References Tab" with "Moodboard Tab" on idea details page
- **Same component** as standalone moodboard page
- **Smart responsive defaults**:
  - **Desktop**: Default to canvas view (infinite workspace, spatial freedom)
  - **Mobile/Tablet**: Default to gallery view (easier touch interaction, no freeform canvas challenges)
  - **User override**: User can manually switch to any view mode, preference saved
- **Benefits**:
  - DRY: One component, one codebase
  - Consistent UX: Same experience everywhere
  - Feature parity: All moodboard features available in all contexts
  - Reduced maintenance: No duplicate code to keep in sync
